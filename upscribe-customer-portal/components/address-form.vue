...
export default {
  ...
  computed: {
    ...
    addressDataFill() {
      const { activeBillingAddress, activeShippingAddress, hasSameAddress} = this
      ...
      const copyShippingAddress = { ...activeShippingAddress }
       ...
       if (this.formName === 'billing-address') {
          return {
            firstName: hasSameAddress ? copyShippingAddress.first_name ? copyShippingAddress.first_name : '' : copyBillingAddress.first_name ? copyBillingAddress.first_name : '',
            lastName: hasSameAddress ? copyShippingAddress.last_name ? copyShippingAddress.last_name : '' : copyBillingAddress.last_name ? copyBillingAddress.last_name : '',
            company: hasSameAddress ? copyShippingAddress.company ? copyShippingAddress.company : '' : copyBillingAddress.company ? copyBillingAddress.company : '',
            address1: hasSameAddress ? copyShippingAddress.address1 ? copyShippingAddress.address1 : '' : copyBillingAddress.address1 ? copyBillingAddress.address1 : '',
            address2: hasSameAddress ? copyShippingAddress.address2 ? copyShippingAddress.address2 : '' : copyBillingAddress.address2 ? copyBillingAddress.address2 : '',
            city: hasSameAddress ? copyShippingAddress.city ? copyShippingAddress.city : '' : copyBillingAddress.city ? copyBillingAddress.city : '',
            country: hasSameAddress ? copyShippingAddress.country ? copyShippingAddress.country : '' : copyBillingAddress.country ? copyBillingAddress.country : '',
            state: hasSameAddress ? copyShippingAddress.province ? copyShippingAddress.province : '' : copyBillingAddress.province ? copyBillingAddress.province : '',
            province: hasSameAddress ? copyShippingAddress.province ? copyShippingAddress.province : '' : copyBillingAddress.province ? copyBillingAddress.province : '',
            provinceCode: hasSameAddress ? copyShippingAddress.province_code ? copyShippingAddress.province_code : '' : copyBillingAddress.province_code ? copyBillingAddress.province_code : '',
            zipcode: hasSameAddress ? copyShippingAddress.zip ? copyShippingAddress.zip : '' : copyBillingAddress.zip ? copyBillingAddress.zip : '',
            phone: hasSameAddress ? copyShippingAddress.phone ? copyShippingAddress.phone : '' :  copyBillingAddress.phone ? copyBillingAddress.phone : '',
        }
       } else {
          return {
  ...
  watch: {
    form(newVal = {}, oldVal = {}) {
      ...
      if (newVal.country && oldVal.country && newVal.country !== oldVal.country) {
        this.form.state = ''
        this.form.province = ''
      }
    },
      ...
      axios(`${API}/zipcode/${zipcode}`)
        .then((res) => {
          if (typeof res.data === 'object') {
            ...
            console.log('error get zipcode', res.data)
          }
        })
        .catch((err) => {
          console.log('err', err)
