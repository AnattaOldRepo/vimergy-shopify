...
<template>
  <drawer-wrap class = "c-darwer__edittingAddress" :show="show" :status="drawerStatus" @close="$emit('close')">
    <div class="c-drawerBillingAddresses c-drawer" >
      <h2 class="c-drawer__title">{{ atc['portal.editBillingAddressDrawerTitle'] || 'Edit Billing Address' }}</h2>

      <p class="c-drawer__subtitle">
         {{ atc['portal.editBillingAddressDrawerPrompt'] || "These products ship every 15 days"}}
      </p>

      <div class = "c-drawer__checkBox c-formInput">
        <p>Same as shipping address<p>

        <div class = "c-drawer__customedCheckbox"
            :class= "{'c-drawer__customedCheckbox--active': hasSameAddress}"
            @click = "toggleAddress"
        >
          <span
              class = "c-drawer__customedCheckboxLine"
              :class = "{'c-drawer__customedCheckboxLine--active': hasSameAddress}"
          >
          </span>
        </div>
      </div>
