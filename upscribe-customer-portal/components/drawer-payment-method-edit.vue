...
export default {
  ...
  destroyed() {
    // reset state in case it was a load state
    this.setLoadActionState(null)
  ...
  methods: {
    ...
    async updatePaymentMethod(updatePaymentData, paymentType) {
      ...
      try {
        const updatedPayment = await this.UPDATE_PAYMENT_METHOD({
          updatePayload: updatePaymentData,
          paymentMethodId,
          paymentType,
          paymentCustomerId,
        })
        ...
        console.error('card/UPDATE_PAYMENT_METHOD error: ', e)
        this.$emit('setDrawerStatus', {
          state: 'FAILURE',
          message: e.message.message ? e.message.message : e.message,
        })
    ...
    showRemoveCardPrompt() {
      ...
      // this.$emit('removePaymentMethod', this.activeEditCard)
    },
  },
}
</script>
