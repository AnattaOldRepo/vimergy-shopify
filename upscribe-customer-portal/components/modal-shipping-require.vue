<template>
  <div class="c-modalShippingRequire">
    <h2 class="c-modalSubscription__shippingTitle">
      {{
        atc['portal.shippingUpdateRequiredDrawerTitle'] ||
    ...
    <p class="c-modalSubscription__shippingSubtitle"
      >{{
        atc['portal.shippingUpdateRequiredDrawerPrompt'] ||
          "The changes you've selected require a shipping method update. Select a new shipping method update to complete your updates."
      }}
    </p>
  ...
  methods: {
    ...
    async handleSelectShippingMethod(shippingMethod) {
      ...
      try {
        ...
        console.error('COMPLETE_SAVED_NEW_CHECKOUT_UPDATE error: ', e)
        this.$emit('updateStatus', {
          status: 'rejected',
